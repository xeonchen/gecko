...
void HttpChannelChild::ReportTrrStateToConsole() {
  ...
  if (StaticPrefs::network_trr_mode() != 2 || mResolvedByTRR || mIsFromCache) {
    return;
  }

  RefPtr<Document> doc;
  nsresult rv = mLoadInfo->GetLoadingDocument(getter_AddRefs(doc));
  if (NS_WARN_IF(NS_FAILED(rv)) || !doc) {
    return;
  }
  ...
  RefPtr<Runnable> runnable =
      NS_NewRunnableFunction("ReportTrrStateToConsoleDelayed", [doc, uri]() {
        nsCString host;
        DebugOnly<nsresult> rv = uri->GetHost(host);
        MOZ_ASSERT(NS_SUCCEEDED(rv));

        AutoTArray<nsString, 1> params = {NS_ConvertUTF8toUTF16(host)};
