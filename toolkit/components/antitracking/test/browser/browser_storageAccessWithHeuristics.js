...
//   const StorageAccessGrantedReason_eOpener = 2;
//   let expectOpener = item => {
...
//   /**
//    * The ContentBlockingLog only preserves the first seen reason, and in this
//    * case, we got eOpener then eOpenerAfterUserInteraction, so the expected
//    * reason will be eOpener.
//    */
//   const StorageAccessGrantedReason_eOpener = 2;
//   let expectOpener = item => {
//     is(item[0], Ci.nsIWebProgressListener.STATE_COOKIES_BLOCKED_TRACKER,
...
add_task(async function testUserInteractionHeuristic2() {
  info("Starting user interaction heuristic test 2...");
  ...
  /**
   * The ContentBlockingLog only preserves the first seen reason, and the
   * expected reason, eOpenerAfterUserInteraction, comes after eOpener.
   */
  const StorageAccessGrantedReason_eOpenerAfterUserInteraction = 1;
  let expectOpenerAfterUserInteraction = item => {
    is(item[0], Ci.nsIWebProgressListener.STATE_COOKIES_BLOCKED_TRACKER,
       "Correct blocking type reported");
    is(item[1], false, "Correct blocking status reported");
    ok(item[2] >= 1, "Correct repeat count reported");
    is(item[3], StorageAccessGrantedReason_eOpenerAfterUserInteraction,
       "Correct granted reason reported");
  };
  ...
  for (let trackerOrigin in log) {
    let originLog = log[trackerOrigin];
    expectOpenerAfterUserInteraction(originLog[3]);
  }
